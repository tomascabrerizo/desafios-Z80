...
;; -------------------------------------------------------------
;; [FUNCION]    : hl_set_last_bagon_position
;; [description]: calcula posicion donde el bagon se debe
;; 		  detener y la guarda en HL
;; [out]        : HL con la posicion
;; -------------------------------------------------------------
hl_set_last_bagon_position:
	ld hl, #c370
	ld a, (barril_offset_p)
	add a, l
	sub a, #2
...
;; [LOOP] loop que cubre el ancho de la pantalla con vias
	...
	init_animation:
		...
		ld de, barril_8x8
		call hl_set_barril_position
		call a_set_barril_count_x2
		ld b, a
		barril_loop:
			call render_sprite_8x8
			inc hl
		...
		call hl_set_last_bagon_position 
